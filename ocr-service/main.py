    ...
    try:
        ...
        if pages_data is not None:
            ...
            for page in pages_data:
                text_lines = [b["text"] for b in page["blocks"]]
                contexts.append(f"TEXTO OCR (NATIVO):\n{' | '.join(text_lines)[:3500]}")
                # Mock items for now, ideally we map bbox to items
        else:
            # Falha na extração nativa, usar OCR
            # Usando PyMuPDF para renderizar a página como imagem
            import fitz
            ...
            for page_num in range(len(doc)):
                ...
                for item in extracted_items:
                    ...
                    if any(char.isdigit() for char in t):
                        ...
                        x_norm = ((box["x1"] + box["x2"]) / 2) / w
                        y_norm = ((box["y1"] + box["y2"]) / 2) / h
