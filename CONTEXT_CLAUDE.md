...
- `e095c67` — **Compatibilidade com Azure AI Studio (GPT-5)**: suporte a gateways de modelo (incluindo model no body) e parametrização dinâmica de `max_completion_tokens` (Antigravity).
- `d1da771` — **Fix JSON GPT-5 Azure**: remoção do parâmetro `response_format: json_object` e `temperature: 0` para compatibilidade com o comportamento de modelos de raciocínio da nova geração Azure (Antigravity).
